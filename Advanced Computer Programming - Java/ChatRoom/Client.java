...
public class Client extends JFrame implements Runnable{
	...
	public void run(){
		if(!isconnect){
			try {
				isconnect = true;
				// get ip and port
				clsocket = new Socket(iptext.getText().toString(),Integer.parseInt(porttext.getText()));
				if(!clsocket.isClosed()){
					
					displayArea.append("Connected"+"\n");
					usertext.setEditable(false);
					openStream(clsocket); // open data input and output stream
					String str = in.readUTF();
					textbox.requestFocus(); 
					out.writeUTF(usertext.getText()+" connected from " + InetAddress.getLocalHost());  //write Client connect msg to server side
					...
					displayArea.append("Connect error, please check whether Server is running and try again later\n");
					isconnect = false;
				}
			} catch (Exception e) {
				displayArea.append("Connect error, please check whether Server is running and try again later\n");
			...
			displayArea.append("You are already connected\n");
		}
	}
	...
	private void send(String s){
		try {
				openStream(clsocket);
				out.writeUTF(usertext.getText() + " : " + s);  //get textbox msg and write to server side
				out.flush();
	...
	public class read implements Runnable{
		...
		public void run() {
			try {
				while(true){
					openStream(clsocket);   // open data input and output stream and buffer input stream
					String s;
					s = in.readUTF();   	// datainputstream read msg from server side
